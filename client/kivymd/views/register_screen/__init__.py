...
from utils.server_http import apt_get_captcha, api_sms_code, api_post_user_me

Builder.load_file(
    os.path.join("views", "register_screen", "register_screen.kv"))
...
class RegisterScreen(MDScreen):
    ...
    def register(self):
        ...
        if not sms_code:
            ...
            self.show_dialog("注册失败", "短信验证码不能为空!")
            return
        password = self.ids.password.text
        if not password:
            self.ids.password.error_color = "red"
            self.ids.password.error = True
            self.show_dialog("注册失败", "密码不能为空!")
            return
        if len(password) < 10 or len(password) > 20:
            self.ids.password.error_color = "red"
            self.ids.password.error = True
            self.show_dialog("注册失败", "密码长度错误!")
            return
        ...
        if password != password_confirm:
            ...
            self.show_dialog("注册失败", "两次密码不一致!")
            return
        try:
            api_post_user_me(phone, sms_code, password)
        except Exception as e:
            self.show_dialog("注册失败", e.args[0])
            return
        dialog = MDDialog(
            MDDialogHeadlineText(
                text="注册成功",

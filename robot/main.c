...
void main(int argc, char **argv) {
	...
	unsigned char tip0[] = "AT+RST\r\n";
	unsigned char tip1[] = "AT\r\n";
	unsigned char tip2[] = "AT+CIPSTART=\"TCP\",\"zhangmingjian180.love\",7922\r\n";
	unsigned char tip3[] = "AT+CIPMODE=1\r\n";
	unsigned char tip4[] = "AT+CIPSTATUS\r\n";
	unsigned char tip5[] = "AT+CIPMODE?\r\n";

    EXE(send(fd, tip1, sizeof(tip1)));
	EXE(send(fd, tip2, sizeof(tip2)));
	EXE(send(fd, tip3, sizeof(tip3)));
	EXE(send(fd, tip4, sizeof(tip4)));
	EXE(send(fd, tip5, sizeof(tip5)));

	flag = set_attr_v(fd, 1, 0);
    if(flag == -1){
        perror("failed to set attribute.");
        return;
    }
